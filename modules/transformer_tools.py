...
def get_tuple_of_locations(locations_col):
    """
    Gets important locations from column of locations.
    In this context used to extract locations
    :param locations_col: Column with the locations
    :return: Lowercase tuple of the words that appear at least 10 times in the column and are longer than 2 characters
    """
    locations_col = locations_col.str.lower()
    locations = locations_col.value_counts() > 10
    locations = list(pd.DataFrame(locations_col.value_counts()[locations]).index.values)
...
def get_locations_by_tuple(locations_col, locations_to_extract):
    """
    Extracts all occurrences of locations from a given column. The locations to extract are given as a list.
    :param locations_col: Column containing the locations
    :param locations_to_extract: Tuple with the locations to be extracted
    :return: Four columns of locations
    """
    locations_col = locations_col.apply(lambda x: [j for j in x.split() if j.lower() in locations_to_extract])
    location_first = locations_col.str[0]
    location_second = locations_col.str[1]
    location_third = locations_col.str[2]
    location_fourth = locations_col.str[3]
    return location_first, location_second, location_third,location_fourth


def remove_word_from_other_column(words_col, words_to_remove):
    """
    Removes word from a given column provided by a second column
    :param words_col: Column where the words should be removed from
    :param words_to_remove: Column containing the words that should be removed
    :return: Original column without the words that should be removed
    """

    return [e.replace(k, '') for e, k in zip(words_col.astype('str'), words_to_remove.astype('str'))]
...
def extract_author(descriptions):
    ...
    # Removing everything non-alphanumeric (symbol set specific for German)
...
def get_topicfilter_list():
    ...
    """
    topic_filter = open(r"../resources/%s" % TopicFilter, "r")
